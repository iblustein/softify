      ...
      <div className="border-b border-slate-200 pb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Analyze catalog compliance warnings and draft safe product metadata suggestions. Zero direct theme or mutation execution paths are allowed.
          </p>
        </div>
        <button
          onClick={fetchCatalogAndWorkspaceData}
          className="inline-flex items-center gap-1 px-3 py-1.5 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 rounded-lg text-3xs font-bold shadow-2xs transition cursor-pointer"
        >
              ...
              <div key={agent.agentId} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[9px] text-slate-400 font-mono mt-3">
                  ...
                  <button
                    ...
                    className="text-indigo-600 hover:text-indigo-800 font-bold uppercase"
                  >
                    Select Agent
                  </button>
                </div>
                ...
                return (
                  <div key={act.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs space-y-3 relative overflow-hidden">
                    ...
                    <div className="bg-slate-50 border border-slate-150 rounded-xl p-3 space-y-2">
                      ...
                      <pre className="text-4xs text-slate-650 text-slate-600 font-mono whitespace-pre-wrap leading-tight max-h-24 overflow-y-auto bg-slate-950 text-emerald-450 text-emerald-400 p-2 rounded-lg border border-slate-200">
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between mt-3 text-3xs">
                      ...
                      {isExecutable && !isRequested && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-3.5 py-1.5 bg-gradient-to-r from-indigo-650 to-indigo-650 from-indigo-600 to-indigo-600 hover:from-indigo-500 hover:to-indigo-500 text-white rounded-lg font-bold shadow-xs transition cursor-pointer leading-none"
                        >
                      ...
                      {!isExecutable && (
                        <span className="text-4xs text-slate-400 font-mono flex items-center gap-1 font-bold">
