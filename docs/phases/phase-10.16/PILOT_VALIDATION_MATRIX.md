...
| **agent_catalog_cleanup** | Run cleanup diagnostic scan. | Generates draft proposed actions only for cleanup-allowed fields such as vendor, productType, status, or tags. | Proposal database changes. | [ ] Pass / Fail | Capped metadata |
| **agent_merchandising_insights** | Run merchandising insights scan. | Returns read-only metrics. Proposal and mutation tools completely blocked. | Gateway allowed/blocked logs. | [ ] Pass / Fail | Zero write access |
| **agent_approval_operations** | Run approvals workspace scan. | Returns read-only approvals overview counts and Funnel statistics. | Analytics overview UI. | [ ] Pass / Fail | Zero write access |
