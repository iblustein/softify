...
- **System AI Engines & Agent Engine Assignment**: Implemented system-managed AI engine registry, sanitized metadata endpoint `GET /api/settings/ai-engines`, safe `POST /api/settings/ai-engines/:engineId/test` connectivity probes, generic `AgentInstallation` assignment schemas, custom React settings controls for real-time engine/model assignment with strict allowlisted boundaries, and dynamic chat prompt routing.
- **CI/CD & Production Smoke Tests**: Full verification suite passed cleanly. Static release checks passed 59/59, local in-process smoke tests passed 34/34, and actual deployed Cloud Run smoke tests passed 10/10.
...
- **Phase 11.2 — Theme Editor AI Agent Pilot Launch**: Focuses on launching the live merchant pilot program specifically for the Theme Editor AI Agent MVP, gathering qualitative merchant feedback, and evaluating onboarding checklist flows.
